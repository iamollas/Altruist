...
class Altruist:
    ...
    def __init__ (self, model, training_data, fi_technique, feature_names, cbi_features):
        """
        Parameters
        ----------
            model: ml model
    ...
    def _prolog_query(self, instance):
        ...
        for fi in range(1,self.fis+1):
            ...
            a = "'prolog_outputs/temp_"+str(fi)+".pl'"
            b = "'Explanation'"
            untruthful_features = []
            os.system('swipl -g "consult('+a+'), untrusted('+b+')" 2>&1 | tee prolog_outputs/temp_out.txt')
            tf = open("prolog_outputs/temp_out.txt","r")
            for x in tf:
                if 'indeed' in x:
    ...
    def _prolog_query_explain(self, instance, fi, truthful_only):
        ...
        a = "'prolog_outputs/temp_"+str(fi+1)+"_explain.pl'"
        b = "'Explanation'"
        
        untruthful_features = []
        os.system('swipl -g "consult('+a+'), untrusted('+b+')" 2>&1 | tee prolog_outputs/temp_out_explain.txt')
        tf = open("prolog_outputs/temp_out_explain.txt","r")
        for x in tf:
            final.append(x[:-1])
    ...
    def _write_pl(self, instance, fi, explain=False):
        ...
        if explain:
            tf = open("base_b.pl","r")
            f = open("prolog_outputs/temp_"+str(fi)+"_explain.pl", "w")
        else:
            tf = open("base_a.pl","r")
            f = open("prolog_outputs/temp_"+str(fi)+".pl", "w")
        for x in tf:
        	f.write(x)
